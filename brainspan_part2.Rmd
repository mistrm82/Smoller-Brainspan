---
title: "Smoller Brainspan Part 2"
output:
  html_document:
    theme: cosmo
    toc: true
...
> In Part I, we verified that the problem of confounding variables exists within the Kang et al developmental expression data that we are using. In the next stages we aim to remove the effects of confounders by applying  various methods which include isva best described in [Teschendorff A.E. et al., 2011](http://bioinformatics.oxfordjournals.org/content/27/11/1496.long) and RUV (remove unwanted variation) a method described in more detail [Gagnon-Bartsch J.A., 2011](http://biostatistics.oxfordjournals.org/content/13/3/539.short). 

ISVA is not inlcuded in the final html report but code remains in the markdown file.
